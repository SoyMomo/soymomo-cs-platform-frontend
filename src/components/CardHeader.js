        ...
        <div style={{ backgroundColor: '#fff', borderRadius: '0.5rem', boxShadow: '0px 2px 4px rgba(0, 0, 0, 0.1)', padding: '1rem', marginBottom: '0.625rem', maxWidth: '100%'}}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center'}}>
                    {props.leftIcon && (
                            <div style={{ backgroundColor: '#603BB0', borderRadius: '0.75rem', padding: '1rem', marginRight: '0.75rem' }}>
                                <img src={props.leftIcon} width={props.leftIconWidth} height={props.leftIconHeight} alt='SoyMomo Logo' />
                            </div>
                        )}
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <h1 style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#603BB0' }}>{props.title}</h1>
                        <p style={{ fontSize: '0.875rem', color: '#603BB0', alignSelf: 'flex-start' }}>{props.subtitle}</p>
                    </div>
                </div>
